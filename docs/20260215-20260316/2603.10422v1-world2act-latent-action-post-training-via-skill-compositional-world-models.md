...
tags: ["query:rob-ai"]
score: 10.0
evidence: 利用技能组合世界模型对VLA策略进行后训练
...
## 摘要
世界模型 (WMs) 已成为后训练视觉-语言-动作 (VLA) 策略的一种极具前景的方法，旨在提高环境变化下的鲁棒性和泛化能力。然而，大多数基于世界模型的后训练方法依赖于像素空间监督，使得策略对像素级伪影和不完美世界模型推演 (rollouts) 产生的幻觉非常敏感。我们推出了 World2Act，这是一个后训练框架，它使用对比匹配目标将 VLA 动作直接与世界模型视频动力学潜变量 (latents) 对齐，从而减少了对像素的依赖。后训练性能与推演质量密切相关，但当前的世界模型在生成任意长度视频方面面临困难，因为它们大多在固定长度的片段上进行训练，而机器人执行持续时间差异很大。为了解决这个问题，我们提出了一种基于大语言模型 (LLM) 的自动技能分解流水线，将高层指令分割为底层提示词。我们的流水线生成了 RoboCasa-Skill 和 LIBERO-Skill，支持在不同任务跨度下保持时间一致性的技能组合世界模型。实验表明，将 World2Act 应用于 GR00T-N1.6 和 Cosmos Policy 等 VLA 策略，在 RoboCasa 和 LIBERO 上取得了最先进的结果，并将真实世界性能提升了 6.7%，增强了具身智能体的泛化能力。
