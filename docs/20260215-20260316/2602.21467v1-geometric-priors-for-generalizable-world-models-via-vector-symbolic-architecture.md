...
tags: ["query:rob-ai"]
score: 8.0
evidence: 通过几何先验实现可泛化的世界模型
tldr: 针对现有世界模型在泛化性和可解释性方面的不足，本文提出了一种基于向量符号架构（VSA）几何先验的通用世界模型。该模型利用可学习的傅里叶全息简化表示（FHRR）将状态和动作映射到具有群结构的复向量空间，并通过元素级复数乘法模拟状态转移。实验表明，该方法在零样本泛化、长时程预测和抗噪性方面显著优于传统MLP模型，为构建结构化、高效且可解释的规划与推理模型提供了新路径。
selection_source: fresh_fetch
motivation: 传统世界模型多采用无结构神经网络，导致其在处理未见状态或动作组合时泛化能力差且缺乏可解释性。
method: 引入向量符号架构（VSA）作为几何先验，利用FHRR编码器将动力学建模为高维复向量空间中的群作用（元素级复数乘法）。
result: "在离散网格世界中，该模型实现了87.5%的零样本泛化准确率，长时程预测准确率提升53.6%，且抗噪能力提高4倍。"
conclusion: 通过在潜空间中引入群论结构，可以显著提升世界模型的泛化性、数据效率和鲁棒性，是实现复杂推理的有效途径。
---

## 摘要
人工智能和神经科学领域的一个关键挑战是理解神经系统如何学习能够捕捉世界底层动力学的表征。大多数世界模型使用无结构的神经网络来表示状态转移函数，这限制了模型的可解释性、样本效率以及对未见状态或动作组合的泛化能力。我们通过一种以向量符号架构（VSA）原理作为几何先验的可泛化世界模型来解决这些问题。我们的方法利用可学习的傅里叶全息简化表征（FHRR）编码器，将状态和动作映射到具有学习到的群结构的高维复向量空间中，并利用逐元素复数乘法对状态转移进行建模。我们形式化了该框架的群论基础，并展示了如何通过训练此类结构化表征使其具有近似不变性，从而直接在潜空间中实现强大的多步组合能力，并在各种实验中表现出优异的泛化性能。在离散网格世界环境中，我们的模型在未见过的状态-动作对上实现了 87.5% 的零样本准确率，在 20 个时间步长的预测中准确率提高了 53.6%，且相对于 MLP 基准模型，其噪声鲁棒性提高了 4 倍。这些结果强调了训练具有潜群结构的表征如何产生可泛化、高数据效率且可解释的世界模型，为构建用于现实世界规划和推理的结构化模型提供了一条有原则的路径。
