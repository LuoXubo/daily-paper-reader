...
motivation: 现有模型倾向于预测绝对的未来状态，缺乏对动作引起的世界知识变化过程的推理。
method: 提出 ΔVLA 框架，包含先验引导的知识提取器、潜变量变化量化模块以及条件变化注意力机制。
result: 在模拟基准测试和真实机器人任务中均取得了最先进的性能，并显著提升了推理效率。
conclusion: 通过建模世界知识的相对变化而非绝对状态，可以更有效地引导机器人进行复杂的操作任务。
---
