...
## 摘要
协调多个自主智能体以探索并服务空间异构需求，需要联合学习未知的空间模式并规划能够最大化任务性能的轨迹。纯基于模型的方法虽然能提供结构化的不确定性估计，但缺乏自适应策略学习；而深度强化学习在缺乏空间先验时，往往面临样本效率低下的问题。本文提出了一种混合信念强化学习（HBRL）框架来解决这一问题。在第一阶段，智能体利用对数高斯考克斯过程（LGCP）构建空间信念，并在具有多步前瞻的路径互信息（PathMI）规划器的指导下执行信息驱动的轨迹。在第二阶段，轨迹控制权被移交给软 Actor-Critic（SAC）智能体，并通过双通道知识迁移进行预热：信念状态初始化提供空间不确定性，而经验回放池播种则提供 LGCP 探索期间生成的演示轨迹。一种方差归一化的重叠惩罚通过共享信念状态实现协同覆盖，允许在高不确定性区域进行合作感知，同时抑制在已充分探索区域的冗余覆盖。该框架在多无人机无线服务提供任务上进行了评估。结果表明，与基准方法相比，该框架的累积奖励提高了 10.8%，收敛速度加快了 38%，消融实验进一步证实了双通道迁移的效果优于单一通道。
