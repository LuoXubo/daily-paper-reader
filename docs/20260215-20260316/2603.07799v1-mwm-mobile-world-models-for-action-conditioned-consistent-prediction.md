...
tags: ["query:rob-ai"]
score: 10.0
evidence: 用于基于规划的导航的移动世界模型
tldr: 针对具身导航中世界模型多步预测易漂移及蒸馏推理不一致的问题，本文提出 MWM 移动世界模型。该模型采用结构预训练与动作一致性（ACC）后训练的两阶段框架，并引入推理一致性状态蒸馏（ICSD）技术，在保证少步扩散推理效率的同时，显著提升了动作条件下的长程预测一致性，为图像目标导航提供了更精准的规划基础。
selection_source: fresh_fetch
motivation: 现有导航世界模型在多步预测时缺乏动作一致性，且扩散模型蒸馏过程往往导致训练与推理之间的不匹配。
method: 提出包含结构预训练与动作一致性后训练的两阶段框架，并引入推理一致性状态蒸馏（ICSD）优化少步推理。
result: 在基准测试和真实任务中，该方法显著提升了视觉保真度、轨迹精度、规划成功率及推理效率。
