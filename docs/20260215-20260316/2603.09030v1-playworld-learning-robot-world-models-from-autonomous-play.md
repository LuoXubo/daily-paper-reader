...
## 摘要
动作条件视频模型为构建能够直接从数据中改进的通用机器人模拟器提供了一条充满前景的路径。然而，尽管在大型机器人数据集上进行了训练，当前的先进视频模型在预测机器人操作中至关重要的物理一致性机器人-物体交互方面仍然面临挑战。为了弥补这一差距，我们提出了 PlayWorld，这是一个简单、可扩展且完全自主的流水线，用于从交互经验中训练高保真视频世界模拟器。与以往依赖于具有成功偏差的人类演示的方法不同，PlayWorld 是第一个能够完全从无监督机器人自博弈中学习的系统，在实现自然可扩展的数据采集的同时，捕捉到了建模真实物体动力学所必需的复杂、长尾物理交互。在多种操作任务上的实验表明，PlayWorld 为富接触交互生成了高质量、物理一致的预测，而这些交互是基于人类采集数据训练的世界模型所无法捕捉的。我们进一步展示了 PlayWorld 在实现细粒度故障预测和策略评估方面的多功能性，相比人类采集的数据，其性能提升高达 40%。最后，我们展示了 PlayWorld 如何在世界模型中实现强化学习，在现实世界部署时将策略性能的成功率提高了 65%。
