...
tags: ["query:rob-ai"]
score: 10.0
evidence: 用于人形机器人运动与操作的层级化世界模型
tldr: MetaWorld-X是一个针对人形机器人全身协同控制（移动与操作）的分层世界模型框架。针对单一策略在处理高自由度系统时存在的梯度干扰和动作不自然问题，该研究采用分而治之策略，通过模仿人类运动先验训练专业专家策略（SEP），并利用视觉语言模型（VLM）引导的智能路由机制（IRM）实现语义驱动的专家组合，显著提升了复杂任务下的动作自然度、稳定性和泛化能力。
selection_source: fresh_fetch
motivation: 传统的单一强化学习策略在处理人形机器人复杂的移动与操作协同任务时，容易出现动作冲突、不自然及泛化性差的问题。
method: 提出MetaWorld-X框架，结合人类运动先验训练专业专家策略，并利用VLM监督的智能路由机制进行语义化的任务分解与执行。
result: 该方法成功实现了生物力学一致的自然动作生成，并在多阶段复杂任务中展现出卓越的组合泛化能力和执行稳定性。
conclusion: 通过分层架构与VLM引导的专家协作，MetaWorld-X为人形机器人实现自然且通用的全身协同控制提供了有效路径。
---
