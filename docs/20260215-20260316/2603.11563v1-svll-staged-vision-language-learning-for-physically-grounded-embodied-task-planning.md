...
tldr: 针对具身任务规划中视觉定位与因果一致性的挑战，本文提出分阶段视觉语言学习（SVLL）框架。该框架通过三阶段训练解耦空间定位与时间推理，并引入Bias-DPO对齐算法，通过注入专家轨迹偏置来解决标准DPO在物理约束下的幻觉问题。实验证明SVLL在AI2-THOR及真实机器人任务中表现优于GPT-4o等模型，显著提升了任务成功率并减少了违规行为。
selection_source: fresh_fetch
motivation: 现有的具身规划训练方法在处理视觉定位与时间推理的耦合时存在过早绑定或优化不稳定的问题，且标准DPO易导致物理上不可行的幻觉。
method: 提出SVLL三阶段框架，先解耦空间与时间学习，再利用Bias-DPO算法在优化偏好间隙的同时强化对专家轨迹的绝对似然约束。
result: 在AI2-THOR基准测试和真实机器人部署中，SVLL的成功率超越了Qwen2.5-VL、GPT-4o和Gemini-2.0-flash等先进模型。
conclusion: 通过分阶段学习和改进的对齐目标，可以有效增强视觉语言模型在物理环境中的接地能力和因果推理稳定性。
---

## 摘要
具身任务规划要求视觉语言模型生成既具有视觉落地性（visually grounded），又在时间上具有因果连贯性的动作序列。然而，现有的训练范式面临着关键的权衡：联合端到端训练往往导致过早的时间绑定，而标准的强化学习方法则面临优化不稳定的问题。为了弥补这一差距，我们提出了分阶段视觉语言学习（SVLL），这是一个用于鲁棒、物理落地具身规划的统一三阶段框架。在前两个阶段，SVLL 将空间落地与时间推理相解耦，在引入序列动作历史之前建立鲁棒的视觉依赖。在最后阶段，我们指出了标准直接偏好优化（DPO）的一个关键局限性，即其纯粹的相对性质——仅优化获胜轨迹与失败轨迹之间的偏好差距，而忽略了对最优路径的绝对似然约束，这往往会导致不安全或幻觉行为。为解决这一问题，我们进一步引入了 Bias-DPO，这是一种新型对齐目标，通过显式最大化真实动作（ground-truth actions）的似然并惩罚过度自信的幻觉，向专家轨迹注入归纳偏置。通过将策略锚定在专家流形上并减轻因果失配，由 Bias-DPO 驱动的 SVLL 确保了对环境示能性（affordances）的严格遵守，并有效抑制了物理上不可能的捷径。最后，在交互式 AI2-THOR 基准测试和真实世界机器人部署上的广泛实验表明，SVLL 在任务成功率方面优于最先进的开源模型（如 Qwen2.5-VL-7B）和闭源模型（如 GPT-4o、Gemini-2.0-flash），同时显著减少了物理约束违规。
