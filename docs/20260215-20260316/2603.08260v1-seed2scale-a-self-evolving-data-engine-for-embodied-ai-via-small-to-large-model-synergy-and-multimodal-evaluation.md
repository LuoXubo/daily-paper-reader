...
## 摘要
现有的数据生成方法面临探索限制、具身鸿沟以及低信噪比等问题，导致在自迭代过程中出现性能退化。为了应对这些挑战，我们提出了 Seed2Scale，这是一种自进化的数据引擎，通过“小模型采集、大模型评估、目标模型学习”的异构协同来克服数据瓶颈。该引擎仅需少至四个种子演示即可启动，并采用轻量级视觉-语言-动作（VLA）模型 SuperTiny 作为专用采集器，利用其强大的归纳偏置在并行环境中进行鲁棒探索。同时，集成了一个预训练的视觉-语言模型（VLM）作为验证器（Verifier），对海量生成的轨迹自主进行成败判定和质量评分。Seed2Scale 有效缓解了模型崩溃，确保了自进化过程的稳定性。实验结果表明，Seed2Scale 展现出显著的可扩展潜力：随着迭代的进行，目标模型的成功率呈现稳健的上升趋势，实现了 131.2% 的性能提升。此外，Seed2Scale 显著优于现有的数据增强方法，为通用具身智能的大规模开发提供了一条可扩展且具有成本效益的路径。项目主页：https://terminators2025.github.io/Seed2Scale.github.io
