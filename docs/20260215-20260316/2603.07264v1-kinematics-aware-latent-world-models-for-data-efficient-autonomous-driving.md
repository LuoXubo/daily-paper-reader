...
tldr: 针对自动驾驶中数据效率低和缺乏物理约束的问题，本文提出了一种运动学感知的潜空间世界模型。该模型在RSSM基础上，将车辆运动学信息整合进观测编码器，并引入几何感知监督来正则化潜状态。这种方法增强了模型对空间结构和物理动态的理解，显著提升了长时预测精度和策略优化稳定性，在仿真实验中表现优于传统基准。
selection_source: fresh_fetch
motivation: 现有的基于世界模型的强化学习在自动驾驶中缺乏对空间和运动学结构的显式建模，导致长时预测不准且数据效率低。
method: 在RSSM框架中引入车辆运动学信息进行物理对齐，并利用几何感知监督来正则化潜状态，使其捕捉任务相关的空间结构。
result: 在驾驶仿真基准测试中，该方法在样本效率和驾驶性能上均优于无模型和纯像素世界模型基准。
