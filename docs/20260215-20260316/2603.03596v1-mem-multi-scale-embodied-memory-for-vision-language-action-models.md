...
tags: ["query:rob-ai"]
score: 10.0
evidence: 视觉-语言-动作模型的多尺度具身记忆
tldr: 针对复杂多阶段机器人任务中单一记忆模式的局限，本文提出多尺度具身记忆（MEM）架构。MEM通过结合视频编码器压缩的短时记忆与基于文本的长时语义记忆，使机器人能够同时处理近期遮挡补偿和长期任务进度管理。该方法支持长达15分钟的长程任务，如烹饪和清理，并显著增强了模型在复杂环境下的上下文自适应能力。
selection_source: fresh_fetch
motivation: 传统的机器人记忆机制难以平衡长时语义抽象与短时细节感知，导致其在处理复杂、长周期的现实任务时表现不佳。
method: 提出一种混合模态记忆架构，利用视频编码器捕捉短时视觉信息，并结合文本描述来记录长时语义概念。
result: MEM使机器人能够成功执行清理厨房和制作三明治等长达15分钟的任务，并能根据上下文灵活调整操作策略。
conclusion: 多尺度、多模态的记忆融合是提升视觉语言动作模型处理长程具身任务能力的关键技术路径。
---
