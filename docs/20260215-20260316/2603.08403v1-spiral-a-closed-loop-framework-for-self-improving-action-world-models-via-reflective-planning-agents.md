...
title_zh: SPIRAL：一种通过反思型规划智能体实现自我改进动作世界模型的闭环框架
...
## 摘要
我们引入了 SPIRAL，这是一个自我改进的规划与迭代反思动作世界建模闭环框架，能够实现以高层语义动作为条件的、可控的长时程视频生成。现有的单次视频生成模型以开环方式运行，通常导致动作执行不完整、语义落地弱以及时间漂移。SPIRAL 将动作世界模型（ActWM）构建为一个“思考-行动-反思”的闭环过程，在显式规划和反馈的指导下逐步进行生成。PlanAgent 将抽象动作分解为以物体为中心的子动作，而 CriticAgent 则评估中间结果，并利用长时记忆引导迭代优化。这种闭环设计自然地支持强化学习（RL）演化优化，从而在更长的时程内提高语义对齐度和时间一致性。我们进一步引入了 ActWM-Dataset 和 ActWM-Bench 用于训练和评估。在多个文本到视频（TI2V）骨干网络上的实验表明，SPIRAL 在 ActWM-Bench 和主流视频生成基准测试中均取得了持续的提升，验证了其有效性。
