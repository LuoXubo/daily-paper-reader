...
tldr: 本研究针对智能体系统中世界模型在一致性与灵活性之间的权衡问题，提出了一种基于DEVS形式化方法的离散事件世界模型生成框架。该方法利用大语言模型（LLM）从自然语言规范中直接合成可执行模型，通过分阶段流水线分离结构推理与逻辑实现。该框架不仅保证了长时程模拟的一致性，还通过事件追踪验证实现了可验证性，为在线执行中的模型按需合成提供了高效可靠的方案。
selection_source: fresh_fetch
motivation: 现有的世界模型在手工模拟器的一致性与神经模型的灵活性之间存在断层，缺乏一种既易于验证又可快速适配的中间方案。
method: 提出一种基于DEVS形式化方法的分阶段LLM生成流水线，将系统组件的结构推理与具体的事件触发及定时逻辑分离。
result: 成功实现了从自然语言规范生成可执行的离散事件模型，并能通过结构化事件追踪进行语义和时序约束的自动化验证。
conclusion: 该方法有效结合了显式模拟器的可靠性与生成式模型的灵活性，为复杂离散事件环境下的智能体规划提供了可验证且高效的建模手段。
---
