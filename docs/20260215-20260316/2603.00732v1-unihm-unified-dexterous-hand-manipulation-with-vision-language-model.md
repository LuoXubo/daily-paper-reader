...
tags: ["query:rob-ai"]
score: 10.0
evidence: 用于灵巧操作的视觉语言动作模型
tldr: UniHM是首个由自由文本指令引导的统一灵巧手操作框架。它通过统一分词器将不同形态的灵巧手映射至共享码本，并利用视觉语言模型从人类交互数据中学习，无需大规模遥操作数据。结合物理引导的动态优化模块，该框架能生成平滑且符合物理规律的操作序列，在多项任务中展现出极强的泛化能力和物理可行性。
selection_source: fresh_fetch
motivation: 针对现有灵巧手操作缺乏开放词汇指令引导以及跨形态泛化能力不足的问题，提出一种统一的控制框架。
method: 开发了统一灵巧手分词器以适配异构形态，并结合视觉语言模型与物理引导的动态细化模块进行序列生成与优化。
result: 在多个数据集和真实世界测试中，UniHM在处理已知及未知物体和轨迹时均达到了最先进的性能水平。
conclusion: 该研究证明了利用视觉语言模型和物理先验可以有效实现跨形态、高泛化性的灵巧手操作规划。
---
