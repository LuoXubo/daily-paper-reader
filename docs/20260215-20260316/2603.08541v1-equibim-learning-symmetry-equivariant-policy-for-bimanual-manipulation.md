...
title_zh: EquiBim：面向双臂操作的学习对称等变策略
authors: "Zhiyuan Zhang, Aditya Mohan, Seungho Han, Wan Shou, Dongyi Wang, Yu She"
date: 2026-03-09
pdf: "https://arxiv.org/pdf/2603.08541v1"
tags: ["query:rob-ai"]
score: 8.0
evidence: 双臂机器人操作的对称等变策略
tldr: 针对双臂机器人模仿学习中忽略物理对称性导致行为不一致的问题，本文提出了 EquiBim 框架。该框架通过在训练中强制执行观测与动作之间的双侧等变性约束，使策略能够显式利用系统的物理对称性。EquiBim 具有模型无关性，支持多种观测模态和动作表示。实验证明，该方法在仿真和真实世界任务中均显著提升了双臂操作的性能和对分布偏移的鲁棒性。
selection_source: fresh_fetch
motivation: 现有的机器人学习方法往往忽略了双臂系统固有的物理对称性，导致在对称观测下产生不一致或非对称的行为。
method: 提出一种模型无关的等变策略学习框架，将物理对称性建模为群作用，并在训练过程中对观测和动作空间施加等变性约束。
result: 在 RoboTwin 仿真平台和真实双臂系统上的实验表明，该方法在多种配置下均能提升操作性能和鲁棒性。
conclusion: 显式引入物理对称性作为归纳偏置是提升双臂机器人模仿学习效果的一种简单且有效的手段。
---

## 摘要
机器人模仿学习在从演示中学习复杂操作行为方面取得了显著成功。然而，许多现有的机器人学习方法没有明确考虑机器人系统的物理对称性，通常导致在对称观测下产生不对称或不一致的行为。这种局限性在双臂操作中尤为突出，因为双侧对称性是机器人形态和许多任务结构所固有的。在本文中，我们介绍了 EquiBim，这是一个用于双臂操作的对称等变策略学习框架，它在训练过程中强制执行观测与动作之间的双侧等变性。我们的方法将物理对称性公式化为观测空间和动作空间上的群作用，并在对称变换下对策略预测施加等变性约束。该框架与模型无关，可以无缝集成到具有多种观测模态和动作表示的广泛模仿学习流水线中，包括基于点云和基于图像的策略，以及末端执行器空间和关节空间的参数化。我们在具有对称运动学的双臂机器人平台 RoboTwin 上评估了 EquiBim，并在仿真中跨多种观测和动作配置对其进行了评估。我们进一步在真实世界的双臂系统上验证了该方法。在仿真和物理实验中，我们的方法在分布偏移下一致地提高了性能和鲁棒性。这些结果表明，明确强制执行物理对称性为双臂机器人学习提供了一种简单而有效的归纳偏置。
